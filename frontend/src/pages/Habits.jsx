/**
 * Habits.jsx — Compact clean cards
 * - Slim single-row habit cards, expand to show 7-day tray
 * - Status: red=not started, orange=in progress, green=complete
 */
...
function scheduleLabel(h) {
  ...
  if (h.targetTimesPerWeek >= 1)        return `${h.targetTimesPerWeek}x/wk`;
    ...
    <div style={{ display:"flex", flexWrap:"wrap", gap:"8px" }}>
      {HABIT_ICON_LIST.map(key => (
        <button key={key} type="button" onClick={() => onChange(key)} className="btn-reset"
          ...
          <div style={{ width:"100%", height:"100%", color }}>{HABIT_ICONS[key]?.(color)}</div>
        </button>
      ))}
    </div>
  );
      ...
      <motion.div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px" }}>
          <div>
            ...
            <div style={{ fontSize:"11px",color:habit.color,fontWeight:600,marginTop:"3px" }}>{today.slice(8,10)}/{today.slice(5,7)}/{today.slice(0,4)}</div>
          </div>
      ...
      <motion.div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"3px" }}>
          {grid.map((day,i) => {
            ...
            return (
              <div key={ds} style={{ aspectRatio:"1",borderRadius:"7px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px",fontWeight:done||isToday?800:400,
                ...
                border:isToday&&!done?`1.5px solid ${habit.color}66`:"none",opacity:future||before?0.3:1 }}>{day}</div>
            );
          })}
        </div>
        <div style={{ display:"flex",gap:"10px",marginTop:"12px",paddingTop:"10px",borderTop:"1px solid var(--border)" }}>
          {[{color:habit.color,label:"Done"},{color:"var(--border-strong)",label:"Missed"}].map(l=>(
            <div key={l.label} style={{ display:"flex",alignItems:"center",gap:"5px",fontSize:"11px",color:"var(--text-muted)" }}>
              <div style={{ width:"9px",height:"9px",borderRadius:"3px",background:l.color }}/>{l.label}
            </div>
          ))}
        </div>
      </motion.div>
...
function StatsModal({ habit, onClose }) {
  ...
  sorted.forEach((d,i)=>{ if(i===0){cur=1;}else{cur=d===addDaysToYMD(sorted[i-1],1)?cur+1:1;} if(cur>best)best=cur; });
      ...
      <motion.div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ display:"flex",justifyContent:"center",marginBottom:"16px" }}>
          <div style={{ position:"relative",width:"90px",height:"90px" }}>
            <svg width="90" height="90" viewBox="0 0 90 90">
              ...
              <circle cx="45" cy="45" r="36" fill="none" stroke={sc} strokeWidth="8"
                strokeDasharray={`${2*Math.PI*36}`} strokeDashoffset={`${2*Math.PI*36*(1-pct30/100)}`}
                strokeLinecap="round" transform="rotate(-90 45 45)" style={{ transition:"stroke-dashoffset 600ms ease" }}/>
            </svg>
        ...
        <div>
          ...
          <div style={{ display:"flex",gap:"2px",alignItems:"flex-end",height:"36px" }}>
            {days30.map(d=>{ const done=doneSet.has(d),before=d<start; return <div key={d} style={{ flex:1,height:done?"100%":before?"12%":"28%",borderRadius:"2px 2px 0 0",background:done?habit.color:before?"var(--border)":"var(--surface-elevated)",opacity:before?0.3:1 }}/>; })}
          </div>
        </div>
...
function HabitCard({ habit, onToggle, onDelete, onEdit, onOpenQty, onOpenAction, onCalendar, onStats }) {
  ...
  return (
    <motion.div layout
      ...
      style={{ borderRadius:"14px",marginBottom:"7px",overflow:"hidden",border:"1px solid var(--border)",background:"var(--surface)",position:"relative" }}>

      {/* Left accent */}
      <div style={{position:"absolute",left:0,top:0,bottom:0,width:"3px",background:`linear-gradient(180deg,${habit.color},${sc}99)`,borderRadius:"3px 0 0 3px"}}/>

      {/* Main row */}
      <div style={{ display:"flex",alignItems:"center",gap:"10px",padding:"10px 10px 10px 14px" }}>
        <HabitIconTile iconKey={habit.icon||"default"} color={habit.color} size={36}/>

        <div style={{ flex:1,minWidth:0 }}>
          <PremiumCompleteTitle complete={doneToday} style={{ fontSize:"14px",fontWeight:600,display:"block",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
            {habit.name}
          </PremiumCompleteTitle>
          <div style={{ display:"flex",alignItems:"center",gap:"5px",marginTop:"2px",flexWrap:"wrap" }}>
            <span style={{ fontSize:"10px",fontWeight:600,color:habit.color,background:`${habit.color}14`,padding:"1px 6px",borderRadius:"999px" }}>
              {scheduleLabel(habit)}
            </span>
            {streak>0&&(
              <span style={{ fontSize:"10px",fontWeight:700,display:"inline-flex",alignItems:"center",gap:"2px",color:"#F5A623" }}>
                <IconFlame size={9} fill="#F5A623"/> {streak}
              </span>
            )}
            <span style={{ fontSize:"10px",fontWeight:700,color:sc }}>{pct}%</span>
          </div>
        </div>
        ...
        <motion.button type="button" whileTap={{scale:0.84}}
          ...
          style={{
            ...
            boxShadow:doneToday?`inset 0 0 0 1.5px ${sc}44`:`0 3px 10px ${habit.color}40`,
            border:doneToday?`1.5px solid ${sc}44`:"1px solid rgba(255,255,255,0.15)",
          }}>
          {doneToday
            ?<motion.svg initial={{scale:0}} animate={{scale:1}} transition={{type:"spring",stiffness:500,damping:20}} width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></motion.svg>
            :<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          }
        </motion.button>

        {/* More */}
        <motion.button type="button" whileTap={{scale:0.88}} onClick={()=>onOpenAction(habit)} className="btn-reset"
          style={{ width:"26px",height:"26px",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text-muted)",flexShrink:0 }}>
...
export default function Habits() {
  ...
  const resetForm = useCallback(()=>{ setFName(""); setFIcon("default"); setFColor(accent); setEditingId(null); setFFreq("daily"); setFRecurDays("1,2,3,4,5"); setFEveryN(2); setFReminder(false); setFRemTime("09:00"); setShowIcons(false); },[accent]);
  const openCreate = useCallback(()=>{ resetForm(); setShowForm(true); },[resetForm]);
  const openEdit = useCallback((h)=>{ setEditingId(h.id); setFName(h.name); setFIcon(h.icon||"default"); setFColor(h.color||accent); setFFreq(h.frequency||"daily"); setFRecurDays((h.recurringDays||[]).length?h.recurringDays.join(","):"1,2,3,4,5"); setFEveryN(h.everyNDays??2); setFReminder(!!h.reminderEnabled); const rt=h.reminderTime; setFRemTime(typeof rt==="string"&&rt.length>=5?rt.slice(0,5):"09:00"); setShowIcons(false); setShowForm(true); },[accent]);
  const closeForm = useCallback(()=>{ setShowForm(false); resetForm(); },[resetForm]);
  const buildPayload = useCallback(()=>({ name:fName.trim(), icon:fIcon, color:fColor, frequency:fFreq, recurringDays:fFreq==="weekly"?fRecurDays.split(/[,\s]+/).map(x=>parseInt(x,10)).filter(n=>!isNaN(n)&&n>=0&&n<=6):[], everyNDays:fFreq==="interval"?Math.max(1,Number(fEveryN)||2):null, reminderEnabled:fReminder, reminderTime:fReminder?fRemTime:null }),[fColor,fEveryN,fFreq,fIcon,fName,fRecurDays,fReminder,fRemTime]);
  const handleSave = useCallback(async()=>{ if(!fName.trim()){toast.error("Enter a habit name");return;} const p=buildPayload(); if(editingId){await updateHabit(editingId,p);toast.success("Habit updated");}else{await addHabit({...p,startDate:localTodayYMD()});toast.success("Habit created");} closeForm(); },[addHabit,buildPayload,editingId,closeForm,updateHabit,fName]);
  const handleQtyLog = useCallback((id,qty,mode)=>{ const today=localTodayYMD(); const h=habits.find(h=>h.id===id); if(!h)return; const done=(h.completedDates||[]).includes(today); if(mode==="replace"){if(qty>0&&!done)toggleHabit(id,today);if(qty===0&&done)toggleHabit(id,today);}else if(!done)toggleHabit(id,today); toast.success(`${h.name} logged`); },[habits,toggleHabit]);
  ...
  return (
    <div style={{ maxWidth:"680px",margin:"0 auto",padding:"16px 14px 100px",color:"var(--text-body)" }}>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px" }}>
        <div>
          <h1 style={{ fontSize:"22px",fontFamily:"var(--font-heading)",letterSpacing:"-0.03em",marginBottom:"2px" }}>Habits</h1>
        ...
        {total>0&&(
          <svg width="44" height="44" viewBox="0 0 46 46" style={{flexShrink:0}}>
            ...
            <text x="23" y="27.5" textAnchor="middle" fill="var(--text-primary)" fontSize="9" fontWeight="800" fontFamily="var(--font-heading)">{doneToday}/{total}</text>
          </svg>
        )}
      </div>
      ...
      <div style={{ display:"flex",gap:"10px",marginBottom:"12px" }}>
        {[{c:"#FF5A5F",l:"Not started"},{c:"#F5A623",l:"In progress"},{c:"#3DD68C",l:"Complete"}].map(s=>(
          <div key={s.l} style={{ display:"flex",alignItems:"center",gap:"4px",fontSize:"10px",fontWeight:600,color:"var(--text-muted)" }}>
            <div style={{ width:"7px",height:"7px",borderRadius:"50%",background:s.c }}/>{s.l}
          </div>
        ))}
      </div>
        ...
        <AnimatePresence>
          {habits.map(h=>(
            <HabitCard key={h.id} habit={h}
              ...
              onEdit={openEdit} onOpenQty={setQtyHabit}
              onOpenAction={setActionHabit} onCalendar={setCalHabit} onStats={setStatsHabit}
            />
          ))}
        </AnimatePresence>
      )}

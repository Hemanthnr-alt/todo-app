...
export default function Habits() {
  ...
  const bestStreak     = habits.length ? Math.max(0, ...habits.map(h => h.streak)) : 0;

  const openModal  = () => { setHabitName(""); setHabitIcon("🧘"); setHabitColor("#ff6b9d"); setShowModal(true); };
  ...
  const toggle = useCallback((id) => {
    setHabits(prev => prev.map(h => {
      ...
      if (done) { toast("Habit unmarked"); return { ...h, completedDates: dates.filter(d => d !== today), streak: Math.max(0, h.streak - 1) }; }
      toast.success(`${h.icon} Done!`);
  ...
  return (
    <div style={{ maxWidth: "760px", margin: "0 auto", padding: "32px 20px", fontFamily: "'DM Sans', sans-serif", color: textColor }}>
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: "28px" }}>
        ...
        <p style={{ fontSize: "13px", color: mutedColor, margin: 0 }}>
          {completedToday}/{habits.length} done today{habits.length > 0 ? " — keep the streak going 🔥" : ""}
        </p>
      </motion.div>
      ...
      {habits.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <h3 style={{ fontSize: "17px", fontWeight: 700, margin: "0 0 6px", color: textColor }}>No habits yet</h3>
            ...
            return (
              <motion.div key={h.id} layout
                ...
                style={{
                  ...
                  background: done ? (isDark ? "rgba(16,185,129,0.08)" : "rgba(16,185,129,0.05)") : cardBg,
                  backdropFilter: "blur(10px)",
                  border: `1px solid ${done ? "rgba(16,185,129,0.22)" : border}`,
                  marginBottom: "10px", transition: "all 0.2s",
                }}
              >
                <div style={{ width: "44px", height: "44px", borderRadius: "14px", background: `${h.color}1a`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px" }}>
                  {h.icon}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: "15px", fontWeight: 600, color: textColor, textDecoration: done ? "line-through" : "none", opacity: done ? 0.6 : 1, marginBottom: "3px" }}>
                    {h.name}
                  </div>
                  <div style={{ display: "flex", gap: "10px" }}>
                    <span style={{ fontSize: "11px", color: h.streak > 0 ? "#f59e0b" : mutedColor }}>🔥 {h.streak} day streak</span>
                    {done && <span style={{ fontSize: "11px", color: "#10b981", fontWeight: 600 }}>✓ Done today</span>}
                  </div>
                </div>
                <motion.div whileTap={{ scale: 0.88 }} onClick={() => toggle(h.id)} style={{
                ...
                <motion.button whileTap={{ scale: 0.9 }} onClick={() => remove(h.id)} style={{
                  width: "28px", height: "28px", borderRadius: "8px",
                  background: "rgba(244,63,94,0.08)", border: "none",
                  cursor: "pointer", color: "#f43f5e", fontSize: "12px", flexShrink: 0,
      ...
      <CenteredModal isOpen={showModal} onClose={closeModal} title="New Habit" maxWidth="400px">
        <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "20px" }}>
            {ICON_OPTIONS.map(ic => (
              <button key={ic} onClick={() => setHabitIcon(ic)} style={{
                ...
                border: ic === habitIcon ? `2px solid #ff6b9d` : `1px solid ${border}`,
                background: ic === habitIcon ? "rgba(255,107,157,0.14)" : iconBtnBg,
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap" }}>
            {COLOR_OPTIONS.map(c => (
              <div key={c} onClick={() => setHabitColor(c)} style={{
          ...
          <div style={{ padding: "12px 14px", borderRadius: "12px", background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)", border: `1px solid ${border}`, display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
            <div style={{ width: "36px", height: "36px", borderRadius: "10px", background: `${habitColor}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px" }}>{habitIcon}</div>
            <div>
              <div style={{ fontSize: "14px", fontWeight: 600, color: isDark ? "#f1f5f9" : "#0f172a" }}>{habitName || "Habit name"}</div>
              <div style={{ fontSize: "11px", color: habitColor, marginTop: "1px" }}>Preview</div>
            </div>
          </div>

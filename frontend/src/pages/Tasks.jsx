...
function fileIcon(m) {
  if(m?.startsWith("image/"))return"🖼️"; if(m==="application/pdf")return"📄";
  if(m?.includes("word"))return"📝"; if(m?.includes("sheet")||m?.includes("excel"))return"📊";
  if(m==="text/plain")return"📃"; if(m?.includes("zip"))return"📦"; return"📎";
}
function fmtSize(b) {
  ...
  return (
    <div style={{marginTop:"8px"}}>
      ...
      <div style={{display:"flex",gap:"5px",marginTop:"5px"}}>
        ...
        <button onClick={add} style={{padding:"4px 9px",borderRadius:"6px",background:"rgba(255,107,157,0.12)",border:"1px solid rgba(255,107,157,0.25)",color:"#ff6b9d",cursor:"pointer",fontSize:"12px"}}>+</button>
      </div>
    </div>
  );
}
...
function TaskCard({ task, categories, onUpdate, onDelete, isDark }) {
  ...
  const cardBg = isDark
    ? overdue ? "rgba(244,63,94,0.07)" : "rgba(20,10,35,0.6)"
    : overdue ? "rgba(244,63,94,0.04)" : "rgba(255,255,255,0.82)";
    ...
    <motion.div layout initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6,scale:.97}}
      ...
      <div style={{padding:"13px 14px",display:"flex",alignItems:"flex-start",gap:"10px"}}>
        <div onClick={()=>onUpdate(task.id,{completed:!task.completed})} style={{
          width:"19px",height:"19px",borderRadius:"5px",flexShrink:0,marginTop:"2px",
          border:`2px solid ${task.completed?"#ff6b9d":(isDark?"rgba(255,255,255,0.18)":"rgba(0,0,0,0.18)")}`,
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:"7px",flexWrap:"wrap",marginBottom:"5px"}}>
            ...
            {dueFmt&&<span style={{fontSize:"10px",fontWeight:500,padding:"2px 7px",borderRadius:"20px",background:overdue?"rgba(244,63,94,0.12)":"rgba(255,107,157,0.1)",color:overdue?"#f43f5e":"#ff6b9d",flexShrink:0}}>📅 {dueFmt}{overdue?" · Overdue":""}</span>}
        ...
        <div style={{display:"flex",gap:"5px",flexShrink:0}}>
          <motion.button whileTap={{scale:.9}} onClick={()=>setExpanded(!expanded)}
            style={{width:"26px",height:"26px",borderRadius:"7px",background:expanded?"rgba(255,107,157,0.14)":(isDark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.04)"),border:"none",cursor:"pointer",fontSize:"11px",color:expanded?"#ff6b9d":mutedColor,display:"flex",alignItems:"center",justifyContent:"center"}}>
            {expanded?"▲":"▼"}
          </motion.button>
          <motion.button whileTap={{scale:.9}} onClick={()=>onDelete(task.id)}
            style={{width:"26px",height:"26px",borderRadius:"7px",background:"rgba(244,63,94,0.09)",border:"none",cursor:"pointer",fontSize:"12px",color:"#f43f5e",display:"flex",alignItems:"center",justifyContent:"center"}}>
            ✕
          </motion.button>
        </div>
      </div>
            ...
            <div style={{padding:"0 14px 12px",borderTop:`1px solid ${isDark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.05)"}`,paddingTop:"10px"}}>
              ...
              <div>
                  ...
                  <div style={{marginTop:"5px",display:"flex",flexDirection:"column",gap:"3px"}}>
                    {task.attachments.map(att=>(
                      <div key={att.id} style={{display:"flex",alignItems:"center",gap:"7px",padding:"5px 9px",borderRadius:"7px",background:isDark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.03)"}}>
                        ...
                        <a href={att.url} target="_blank" rel="noreferrer" style={{flex:1,fontSize:"11px",color:"#ff6b9d",textDecoration:"none",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{att.originalName}</a>
                        <span style={{fontSize:"10px",color:mutedColor}}>{fmtSize(att.size)}</span>
                        <button onClick={()=>delAttachment(att.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#f43f5e",fontSize:"10px"}}>✕</button>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <button onClick={()=>fileRef.current?.click()} disabled={uploading}
                  style={{marginTop:"6px",padding:"4px 10px",borderRadius:"7px",background:"rgba(255,107,157,0.09)",border:"1px dashed rgba(255,107,157,0.3)",color:"#ff6b9d",cursor:"pointer",fontSize:"11px",fontFamily:"inherit"}}>
...
export default function Tasks() {
  ...
  const textColor  = isDark ? "#f1f5f9"                : "#0f172a";
  const mutedColor = isDark ? "rgba(241,245,249,0.45)" : "rgba(15,23,42,0.45)";
  const cardBg     = isDark ? "rgba(20,10,35,0.6)"     : "rgba(255,255,255,0.82)";
  const border     = isDark ? "rgba(255,107,157,0.1)"  : "rgba(255,107,157,0.15)";
  const inputBg    = isDark ? "rgba(255,255,255,0.07)" : "#ffffff";
  ...
  // ── Filter pill ──────────────────────────────────────────────────────────
  const Pill = ({ active, color="#ff6b9d", onClick, children }) => (
    <button onClick={onClick} style={{
      padding:"4px 11px", borderRadius:"99px",
      border:`1.5px solid ${active ? color : (isDark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.1)")}`,
      ...
      onMouseLeave={e => { if(!active){ e.currentTarget.style.borderColor=isDark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.1)"; e.currentTarget.style.color=mutedColor; }}}
    >{children}</button>
  );

  // ── Filter row label ─────────────────────────────────────────────────────
  const FilterLabel = ({ children }) => (
    <span style={{fontSize:"10px",fontWeight:700,color:mutedColor,textTransform:"uppercase",letterSpacing:"0.07em",flexShrink:0,alignSelf:"center"}}>
      {children}
    </span>
  );
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"24px",flexWrap:"wrap",gap:"10px"}}>
        <div>
          <h1 style={{fontSize:"28px",fontWeight:800,margin:0,letterSpacing:"-0.04em"}}>
            My <span style={{background:"linear-gradient(135deg,#ff6b9d,#c084fc)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Tasks</span>
          </h1>
        ...
        <motion.button whileHover={{scale:1.04}} whileTap={{scale:.97}} onClick={()=>setShowAddForm(!showAddForm)}
          style={{padding:"9px 18px",borderRadius:"12px",background:"linear-gradient(135deg,#ff6b9d,#c084fc)",border:"none",color:"white",cursor:"pointer",fontSize:"13px",fontWeight:700,boxShadow:"0 4px 16px rgba(192,132,252,0.35)",fontFamily:"inherit"}}>
          + New Task
        </motion.button>
      </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"9px"}}>
                ...
                <div style={{display:"flex",gap:"7px",justifyContent:"flex-end"}}>
                  ...
                  <motion.button whileTap={{scale:.97}} onClick={handleAddTask} disabled={addingTask}
                    style={{padding:"8px 18px",borderRadius:"10px",background:"linear-gradient(135deg,#ff6b9d,#c084fc)",border:"none",color:"white",cursor:"pointer",fontSize:"13px",fontWeight:700,fontFamily:"inherit"}}>
      ...
      <CenteredModal isOpen={showCatModal} onClose={()=>setShowCatModal(false)} title="New Category" maxWidth="360px">
        <div style={{fontFamily:"'DM Sans',sans-serif"}}>
          ...
          <div style={{display:"flex",gap:"7px"}}>
            ...
            <button onClick={handleAddCat} style={{flex:1,padding:"10px",borderRadius:"10px",background:"linear-gradient(135deg,#ff6b9d,#c084fc)",border:"none",color:"white",cursor:"pointer",fontSize:"13px",fontWeight:700,fontFamily:"inherit"}}>
              Create
            </button>
          </div>
        </div>

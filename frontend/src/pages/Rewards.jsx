...
const BADGES = [
  ...
  { id:"no_overdue",    icon:"⚡", name:"Always On Time",  desc:"Complete all today's tasks on time",cat:"tasks",  check:(t)=>t.dueToday>0&&t.overdue===0},
...
const CAT_LABELS = {
  tasks:"Tasks", habits:"Habits", org:"Organisation", special:"Special",
};
...
export default function Rewards() {
  ...
  // Check for early bird / night owl on task completion
  useEffect(() => {
  ...
  const earned = useMemo(() => BADGES.filter(b => b.check(ctx)), [tasks, habits, categories]);
  const locked = useMemo(() => BADGES.filter(b => !b.check(ctx)), [tasks, habits, categories]);

  const filtered = filter === "all"
    ? BADGES
    : filter === "earned"
    ? earned
    : locked.filter(b => b.cat === filter || filter === "locked");

  const filteredDisplay = filter === "earned" ? earned
  ...
  const totalHabitDays = habits.reduce((sum,h) => sum+(h.completedDates?.length||0), 0);
    ...
    <div style={{ maxWidth:"900px", margin:"0 auto", padding:"24px 16px", fontFamily:"'DM Sans',sans-serif", color:textColor }}>
      ...
      <div style={{ marginBottom:"24px" }}>
        <h1 style={{ fontSize:"clamp(22px,5vw,28px)", fontWeight:800, margin:"0 0 4px", letterSpacing:"-0.04em" }}>
          <span style={{ background:`linear-gradient(135deg,${ac},${ac}aa)`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Rewards</span>
        </h1>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:"10px", marginBottom:"20px" }}>
        {[
          ...
          { label:"Best Streak",    value:`${longestStreak}d`,                 color:"#f59e0b",   icon:"🔥" },
      ...
      {habits.length > 0 && (
        <div style={{ background:cardBg, borderRadius:"16px", border:`1px solid ${border}`, padding:"16px", marginBottom:"20px" }}>
          <div style={{ fontSize:"11px", fontWeight:700, color:mutedColor, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:"12px" }}>Habit Streaks</div>
          {habits.slice(0,5).map(h => {
            const pct = longestStreak > 0 ? Math.min(100,(h.streak||0)/Math.max(longestStreak,7)*100) : 0;
            return (
              <div key={h.id} style={{ marginBottom:"10px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"5px" }}>
                  ...
                  <span style={{ fontSize:"11px", fontWeight:700, color:h.color }}>🔥 {h.streak||0}</span>
                </div>
                <div style={{ height:"4px", background:isDark?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.07)", borderRadius:"2px", overflow:"hidden" }}>
                  <motion.div initial={{width:0}} animate={{width:`${pct}%`}} transition={{duration:0.7}}
                    style={{ height:"100%", background:`linear-gradient(90deg,${h.color},${h.color}aa)`, borderRadius:"2px" }}/>
                </div>
              </div>
            );
          })}
      ...
      <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"16px" }}>
        {[
          ...
          <button key={f.v} onClick={() => setFilter(f.v)}
            style={{ padding:"5px 12px", borderRadius:"99px", border:`1.5px solid ${filter===f.v?f.c:border}`, background:filter===f.v?`${f.c}15`:"transparent", color:filter===f.v?f.c:mutedColor, cursor:"pointer", fontSize:"11px", fontWeight:filter===f.v?700:400, fontFamily:"inherit", transition:"all 0.13s" }}>
            {f.l}
          </button>
        ))}
      </div>
      ...
      {/* Badge grid */}
      <motion.div layout style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:"10px" }}>
        <AnimatePresence>
          {filteredDisplay.map((badge, i) => {
            ...
            return (
              <motion.div key={badge.id}
                ...
                transition={{delay:i*0.03}} whileHover={{y:-3,scale:1.02}} whileTap={{scale:0.97}}
                onClick={() => setSelected(selected?.id===badge.id ? null : badge)}
                style={{
                  padding:"16px 14px", borderRadius:"16px", cursor:"pointer",
                  background:isEarned ? (isDark?`${catColor}12`:cardBg) : cardBg,
                  backdropFilter:"blur(12px)",
                  border:`1.5px solid ${isEarned?`${catColor}40`:border}`,
                  boxShadow:isEarned?`0 4px 20px ${catColor}20`:"none",
              ...
              >
                ...
                <div style={{ fontSize:"12px", fontWeight:700, color:isEarned?textColor:mutedColor, marginBottom:"3px", lineHeight:1.3 }}>{badge.name}</div>
                <div style={{ fontSize:"10px", color:mutedColor, lineHeight:1.4 }}>{badge.desc}</div>
                {isEarned && (
      ...
      {filteredDisplay.length === 0 && (
        <div style={{ textAlign:"center", padding:"48px 20px", color:mutedColor }}>
        ...
        {selected && (
          <>
            ...
            <motion.div
              ...
              style={{ position:"fixed", left:"50%", top:"50%", transform:"translate(-50%,-50%)", zIndex:2001, width:"min(320px,90vw)", background:isDark?"rgba(10,8,20,0.98)":"rgba(255,255,255,0.98)", backdropFilter:"blur(24px)", borderRadius:"24px", border:`1px solid ${border}`, padding:"28px 24px", textAlign:"center", fontFamily:"'DM Sans',sans-serif" }}>
              <div style={{ fontSize:"52px", marginBottom:"14px" }}>{selected.icon}</div>
              <h3 style={{ fontSize:"18px", fontWeight:800, color:textColor, margin:"0 0 6px", letterSpacing:"-0.03em" }}>{selected.name}</h3>
              <p style={{ fontSize:"13px", color:mutedColor, margin:"0 0 16px", lineHeight:1.6 }}>{selected.desc}</p>
              <div style={{ display:"inline-flex", alignItems:"center", gap:"6px", padding:"6px 14px", borderRadius:"10px", background:`${CAT_COLORS[selected.cat]}15`, color:CAT_COLORS[selected.cat], fontSize:"12px", fontWeight:700 }}>
                {earned.some(b=>b.id===selected.id) ? "✓ Earned" : "🔒 Locked"}
              </div>
              <button onClick={() => setSelected(null)}
                style={{ display:"block", width:"100%", marginTop:"16px", padding:"11px", borderRadius:"12px", background:isDark?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.06)", border:`1px solid ${border}`, color:mutedColor, cursor:"pointer", fontSize:"13px", fontFamily:"inherit" }}>
                Close
              </button>
            </motion.div>
          </>

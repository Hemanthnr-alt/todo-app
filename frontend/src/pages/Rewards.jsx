...
const BADGES = [
  ...
  { id:"no_overdue",     icon:"⚡", name:"Always On Time",   desc:"0 overdue tasks while having tasks due",  cat:"tasks",  check:(t)=>t.dueToday>0&&t.overdue===0             },
...
const CAT_COLORS = { tasks:"#ff6b9d", habits:"#10b981", org:"#8b5cf6", special:"#f59e0b" };
...
export default function Rewards() {
  ...
  const pageBg     = isDark ? "rgba(8,6,16,0.0)"       : "rgba(0,0,0,0.0)";
  ...
  return (
    <div style={{ maxWidth:"900px",margin:"0 auto",padding:"24px 16px",fontFamily:"'DM Sans',sans-serif",color:textColor }}>
              ...
              <div key={h.id} style={{ marginBottom:"10px" }}>
                ...
                <div style={{ height:"4px",background:isDark?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.07)",borderRadius:"2px",overflow:"hidden" }}>
                  <motion.div initial={{width:0}} animate={{width:`${pct}%`}} transition={{duration:0.7}}
                    style={{ height:"100%",background:`linear-gradient(90deg,${h.color||ac},${h.color||ac}aa)`,borderRadius:"2px" }}/>
                </div>
              </div>
            );
          })}
      ...
      {/* Badge grid */}
      <motion.div layout style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:"10px" }}>
        <AnimatePresence>
          {filteredDisplay.map((badge,i)=>{
            ...
            return (
              <motion.div key={badge.id}
                ...
                style={{
                  padding:"16px 14px",borderRadius:"16px",cursor:"pointer",textAlign:"center",
      ...
      {/* ── Badge detail overlay — FIXED: truly centered ── */}
      <AnimatePresence>
        {selected && (
          <>
            <motion.div

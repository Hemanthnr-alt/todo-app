...
import { requestNotificationPermission, checkPermissionStatus, sendNotification } from "../services/notifications";
...
function loadSettings() {
  try {
    ...
    return { notifications:{...DEFAULTS.notifications,...s.notifications}, appearance:{...DEFAULTS.appearance,...s.appearance} };
  ...
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"13px 0", borderBottom:`1px solid ${isDark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.05)"}` }}>
      ...
      <button onClick={() => onChange(!value)}
        style={{ width:"44px", height:"24px", borderRadius:"12px", cursor:"pointer", background:value?`linear-gradient(135deg,${accent},${accent}cc)`:(isDark?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.1)"), position:"relative", transition:"background 0.22s", flexShrink:0, border:"none", outline:"none", boxShadow:value?`0 2px 10px ${accent}44`:"none" }}>
...
function ChangePassword({ isDark, textColor, mutedColor, border, inputBg, accent }) {
  ...
  const iStyle = { width:"100%", padding:"11px 14px", borderRadius:"10px", border:`1px solid ${border}`, background:inputBg, color:textColor, fontSize:"13px", fontFamily:"inherit", outline:"none", boxSizing:"border-box" };
  ...
  return (
    <div style={{ marginTop:"14px" }}>
      <button onClick={() => setShow(!show)}
        style={{ width:"100%", padding:"11px 14px", borderRadius:"12px", border:`1px solid ${border}`, background:show?`${accent}10`:(isDark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.03)"), color:show?accent:textColor, cursor:"pointer", fontSize:"13px", fontWeight:600, fontFamily:"inherit", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        {show && (
          <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}} style={{overflow:"hidden"}}>
            <div style={{ padding:"12px 0 0", display:"flex", flexDirection:"column", gap:"9px" }}>
              {[{l:"Current password",v:current,s:setCurrent},{l:"New password",v:newPw,s:setNewPw},{l:"Confirm new",v:confirm,s:setConfirm}].map(({l,v,s}) => (
                <input key={l} type={showPw?"text":"password"} placeholder={l} value={v} onChange={e=>s(e.target.value)} style={iStyle} onFocus={e=>e.target.style.borderColor=accent} onBlur={e=>e.target.style.borderColor=border}/>
              ))}
              <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                <input type="checkbox" checked={showPw} onChange={e=>setShowPw(e.target.checked)} style={{ accentColor:accent, cursor:"pointer" }}/>
                <span style={{ fontSize:"12px", color:mutedColor }}>Show passwords</span>
              </div>
              <motion.button whileTap={{scale:0.97}} onClick={handle} disabled={saving}
                style={{ padding:"11px", borderRadius:"10px", background:`linear-gradient(135deg,${accent},${accent}cc)`, border:"none", color:"white", cursor:saving?"not-allowed":"pointer", fontSize:"13px", fontWeight:700, fontFamily:"inherit", opacity:saving?0.75:1 }}>
...
export default function AppSettings({ isOpen, onClose }) {
  ...
  const upd = (cat, key, val) => setSettings(p => ({...p, [cat]:{...p[cat],[key]:val}}));
  ...
  const handleEnableNotif = async () => {
    ...
    if (granted) {
      ...
      await sendNotification({ title:"Thirty — Notifications enabled", body:"You'll now receive task reminders." });
    } else {
  ...
  const toggleNetwork = (val) => {
    ...
    toast(val ? "Internet access enabled" : "Internet access disabled — fully offline mode");
  };
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      a.href = url; a.download = `thirty-backup-${new Date().toISOString().slice(0,10)}.json`;
  ...
  const handleRestore = async (e) => {
    ...
    try {
      ...
      toast.success("Data restored! Restart to see changes ✓");
      e.target.value = "";
  ...
  const bg         = isDark ? "rgba(8,5,18,0.98)"     : "rgba(250,251,253,0.98)";
      ...
      {isOpen && (
        <>
          ...
          <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}}
            ...
            style={{ position:"fixed", top:0, right:0, height:"100dvh", width:"min(420px,100vw)", background:bg, backdropFilter:"blur(28px)", borderLeft:`1px solid ${border}`, boxShadow:"-8px 0 56px rgba(0,0,0,0.2)", zIndex:1001, display:"flex", flexDirection:"column", fontFamily:"'DM Sans',sans-serif" }}>
            ...
            <div style={{ padding:"18px 22px 16px", borderBottom:`1px solid ${border}`, display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"3px" }}>
                  <div style={{ width:"28px", height:"28px", borderRadius:"8px", background:`linear-gradient(135deg,${ac},${ac}cc)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"10px", fontWeight:900, color:"white" }}>T</div>
              ...
              <button onClick={onClose}
                style={{ width:"32px", height:"32px", borderRadius:"9px", background:isDark?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.06)", border:`1px solid ${border}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:mutedColor, fontSize:"14px" }}>✕</button>
            </div>
                    ...
                    <div>
                        ...
                        {[{v:"dark",l:"🌙 Dark",d:"Easy on the eyes"},{v:"light",l:"☀️ Light",d:"Bright & clean"}].map(opt => {
                          ...
                          return (
                            <button key={opt.v} onClick={() => { if (!active) toggleTheme(); }}
                              style={{ padding:"12px", borderRadius:"12px", border:`1.5px solid ${active?ac:border}`, background:active?`${ac}12`:cardBg, color:active?ac:textColor, cursor:"pointer", textAlign:"left", fontFamily:"inherit", transition:"all 0.18s" }}>
                      ...
                      <div style={{ display:"flex", gap:"8px", flexWrap:"wrap", marginBottom:"22px" }}>
                        {ACCENT_PRESETS.map(p => (
                          <button key={p.value} onClick={() => changeAccent(p.value)}
                            style={{ width:"32px", height:"32px", borderRadius:"50%", background:p.value, border:accent===p.value?"3px solid white":"2px solid transparent", boxShadow:accent===p.value?`0 0 0 2px ${p.value}, 0 4px 12px ${p.value}55`:"none", cursor:"pointer", transition:"all 0.15s" }}
                            title={p.name}/>
                        ))}
                        <div style={{ position:"relative" }}>
                          <input type="color" value={accent} onChange={e => changeAccent(e.target.value)}
                            style={{ width:"32px", height:"32px", borderRadius:"50%", border:`2px solid ${border}`, cursor:"pointer", padding:0, background:"transparent" }} title="Custom"/>
                        </div>
                      </div>

                      <div style={{ background:cardBg, borderRadius:"12px", padding:"0 14px", border:`1px solid ${border}` }}>
                        <Toggle label="Compact View"         desc="Reduce spacing"                 value={settings.appearance.compactView}       onChange={v => upd("appearance","compactView",v)}      isDark={isDark} accent={ac}/>
                      ...
                      <div style={{ padding:"14px 15px", borderRadius:"14px", background:notifGranted?"rgba(16,185,129,0.08)":"rgba(245,158,11,0.08)", border:`1px solid ${notifGranted?"rgba(16,185,129,0.25)":"rgba(245,158,11,0.25)"}`, marginBottom:"16px", display:"flex", justifyContent:"space-between", alignItems:"center", gap:"12px" }}>
                        <div>
                          ...
                          <div style={{ fontSize:"11px", color:mutedColor, marginTop:"3px" }}>
                            {notifGranted ? "You'll receive reminders" : "Tap Enable to allow"}
                          </div>
                        </div>
                        {!notifGranted && (
                          <button onClick={handleEnableNotif}
                            style={{ padding:"8px 14px", borderRadius:"10px", background:"linear-gradient(135deg,#f59e0b,#f97316)", border:"none", color:"white", cursor:"pointer", fontSize:"12px", fontWeight:700, fontFamily:"inherit", flexShrink:0 }}>Enable</button>
                        )}
                      </div>

                      <div style={{ background:cardBg, borderRadius:"12px", padding:"0 14px", border:`1px solid ${border}`, marginBottom:"16px" }}>
                        <Toggle label="Push Notifications"  desc="Alerts when tasks are due" value={settings.notifications.browser && notifGranted} onChange={v => { if (v && !notifGranted) handleEnableNotif(); else upd("notifications","browser",v); }} isDark={isDark} accent={ac}/>
                        <Toggle label="Due Date Reminders"  desc="Alert before tasks are due" value={settings.notifications.dueReminders} onChange={v => upd("notifications","dueReminders",v)} isDark={isDark} accent={ac}/>
                      </div>
                    ...
                    <div>
                      ...
                      {NATIVE && (
                        <div style={{ background:cardBg, borderRadius:"12px", padding:"0 14px", border:`1px solid ${border}`, marginBottom:"16px" }}>
                          <Toggle
                            ...
                            desc="Allow app to sync with server. Turn off for fully offline mode."
                            value={networkOn}
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:"8px", marginBottom:"20px" }}>
                        <motion.button whileTap={{scale:0.97}} onClick={handleBackup} disabled={backingUp}
                          style={{ padding:"12px 16px", borderRadius:"12px", background:`linear-gradient(135deg,${ac},${ac}cc)`, border:"none", color:"white", cursor:backingUp?"not-allowed":"pointer", fontSize:"13px", fontWeight:700, fontFamily:"inherit", display:"flex", alignItems:"center", gap:"10px", justifyContent:"center", opacity:backingUp?0.7:1 }}>
                          <span>⬇</span> {backingUp?"Preparing…":"Export Backup (.json)"}
                        </motion.button>

                        <label style={{ padding:"12px 16px", borderRadius:"12px", background:cardBg, border:`1px solid ${border}`, color:textColor, cursor:"pointer", fontSize:"13px", fontWeight:600, fontFamily:"inherit", display:"flex", alignItems:"center", gap:"10px", justifyContent:"center" }}>
                          <span>⬆</span> Import Backup
                          <input type="file" accept=".json" onChange={handleRestore} style={{ display:"none" }}/>
                        </label>
                      </div>

                      <div style={{ padding:"14px", borderRadius:"12px", background:"rgba(59,130,246,0.06)", border:"1px solid rgba(59,130,246,0.18)" }}>
                        ...
                        <div style={{ fontSize:"11px", color:mutedColor, lineHeight:1.7 }}>
                          {NATIVE
                            ? "APK: all data stored on your device. Export regularly to back up."
                            : "Web: data synced to our secure servers. Export for an offline copy."
                          }
                        </div>
                      </div>

                      {/* Danger zone */}
                      <div style={{ marginTop:"16px", padding:"14px", borderRadius:"12px", background:"rgba(244,63,94,0.06)", border:"1px solid rgba(244,63,94,0.15)" }}>
                        ...
                        <button onClick={() => { if (window.confirm("Clear all local data?")) { localStorage.clear(); toast("Local data cleared"); } }}
                          style={{ padding:"8px 14px", borderRadius:"8px", background:"rgba(244,63,94,0.1)", border:"1px solid rgba(244,63,94,0.2)", color:"#f43f5e", cursor:"pointer", fontSize:"12px", fontWeight:600, fontFamily:"inherit" }}>
                          Clear local data
                        </button>
                      </div>
                    </div>
            ...
            <div style={{ padding:"14px 22px", borderTop:`1px solid ${border}`, display:"flex", gap:"8px", flexShrink:0, background:isDark?"rgba(0,0,0,0.2)":"rgba(255,255,255,0.4)" }}>
              <button onClick={() => { setSettings(structuredClone(DEFAULTS)); localStorage.removeItem("tp_settings"); toast("Reset to defaults"); }}
                style={{ flex:1, padding:"10px", borderRadius:"10px", border:`1px solid ${border}`, background:"transparent", color:mutedColor, cursor:"pointer", fontSize:"12px", fontWeight:600, fontFamily:"inherit" }}>Reset</button>
              <button onClick={save} disabled={saving}
                style={{ flex:2, padding:"10px", borderRadius:"10px", background:`linear-gradient(135deg,${ac},${ac}cc)`, border:"none", color:"white", cursor:saving?"not-allowed":"pointer", fontSize:"13px", fontWeight:700, fontFamily:"inherit", boxShadow:`0 4px 14px ${ac}44`, opacity:saving?0.75:1 }}>
